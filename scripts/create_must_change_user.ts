...
async function createTestUser() {
  try {
    const email = 'newworker@company.com';
    ...
    if (users.length > 0) {
      ...
      console.log(`User found (ID: ${userId}). Updating password and must_change_password flag...`);
      
      await query(
        `UPDATE ${table('users')} SET must_change_password = true, updated_at = current_timestamp() WHERE user_id = :userId`,
        { userId }
      );
    } else {
      ...
      await query(
        `INSERT INTO ${table('users')} (user_id, email, username, first_name, last_name, role, status, must_change_password, created_at, updated_at) 
         VALUES (:userId, :email, 'newworker', 'New', 'Worker', :role, 'ACTIVE', true, current_timestamp(), current_timestamp())`,
        { userId, email, role }
      );
    }
